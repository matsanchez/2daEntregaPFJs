...
cartSection.innerHTML += tableEmpty;
    ...
    const cartContainer = `
            ...
            for (let i = 0; i < cart.length; i++) {
                ...
                const cartItem = `
                    <tr id=${id}>
                        <th scope="row"><button class="trashItem"></button></th>
                        <td><img src=${img} class="imgProductsCart" alt="${name}"></td>
                        <td>${name}</td>
                        <td>
                            <button id="${name}" class="btn btn-danger __btnItem m-3 p-1">-</button>
                            <p>${cantidad}</p>
                            <button id="${name}" class="btn btn-success __btnItem m-3 p-1">+</button></td>
...
document.addEventListener('click', (e)=>{
    if(e.target && e.target.matches("button.__btnItem")){
        const id = e.target.id
        const product = cart.filter(prod => prod.name == id);
        e.target.textContent == "+" ? product[0].cantidad ++ : product[0].cantidad--
        const cartFilter = cart.filter(prod=> prod.name != id)
        cart = [...cartFilter, ...product]
        localStorage.setItem('cart', JSON.stringify(cart));
        const quantities = e.target.parentNode.childNodes[3]
        quantities.innerHTML = product[0].cantidad
        const price = e.target.parentNode.parentNode.childNodes[11]
...
document.addEventListener("click", (e)=>{
    if(e.target && e.target.matches("button#buyBtn")){
        document.getElementById('buyBtn').setAttribute('data-toggle', "modal")
        document.getElementById('buyBtn').setAttribute('data-bs-toggle',"modal")
        document.getElementById('buyBtn').setAttribute('data-bs-target', "#miModal")
